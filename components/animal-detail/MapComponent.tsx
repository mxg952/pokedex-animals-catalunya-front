...
export default function MapCard({ animal, isLocked }: MapCardProps) {
  ...
  // Función para determinar si el URL es de embed
  const isEmbedUrl = (url: string): boolean => {
    return url.includes('/embed') || url.includes('!1m') || url.includes('!2m') || url.includes('!3m')
  }
          ...
          <>
              ...
              {isLocked ? (
                <div className="h-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
                  <div className="text-center p-6">
                    ...
                    <h4 className="font-semibold text-blue-900 mb-2">Mapa bloquejat</h4>
                    <p className="text-sm text-blue-700">
                      Desbloqueja l'animal per veure la ubicació
                    </p>
                  </div>
                </div>
              ) : (
                <>
                  {/* Mapa embebido - USAR DIRECTAMENTE el URL de la BD */}
                  <iframe
                    src={animal.mapUrl}
                    width="100%"
                    height="100%"
                  ...
                  {/* Overlay para abrir en Google Maps */}
                  <a 
                    href={getNormalGoogleMapsUrl(animal.mapUrl)}
                    target="_blank"
            ...
            {!isLocked && (
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 w-full justify-center bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium"
              >
